...
- **DMARC para el correo del dominio.** `derechos@` es la dirección a la que escriben los editores, y el dominio no tiene DMARC (`_dmarc.antoniolopezsanchez.art` no existe, comprobado el 21 de septiembre de 2026). Sin él es más fácil falsificar un correo que parezca salir de `derechos@`. En get.art, un TXT en `_dmarc` con `v=DMARC1; p=none;` para empezar a vigilar sin romper nada. El modelo a copiar es el de ernestocisneros.art, que ya lo tiene en `p=none` con los informes (`rua`) a su correo. **No subir a `quarantine` ni a `reject`** hasta confirmar desde qué servicio se envía de verdad con esas direcciones: el SPF actual solo autoriza a ImprovMX, así que un envío desde Gmail con la dirección del dominio fallaría y acabaría en spam.

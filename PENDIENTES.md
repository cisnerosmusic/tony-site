...
- **Verificar el dominio en la cuenta de GitHub. Lo más urgente de esta lista.** Sigue sin hacer: el 21 de septiembre de 2026 `_github-pages-challenge-cisnerosmusic.antoniolopezsanchez.art` no existe en el DNS. Protege contra que otra cuenta de GitHub se quede con el dominio si algún día el sitio se despublica mientras el DNS sigue apuntando a GitHub, que es justo como está (los cuatro registros A de GitHub, y `www` a `cisnerosmusic.github.io`). Lo tiene que hacer Ernesto, son unos minutos:
  1. En GitHub, con la cuenta `cisnerosmusic`: foto de perfil > **Settings** > **Pages** (columna izquierda, bajo *Code, planning, and automation*) > **Add a domain** > `antoniolopezsanchez.art`.
  2. GitHub enseña un registro TXT: nombre `_github-pages-challenge-cisnerosmusic` y un valor largo.
  3. El DNS del dominio está en get.art (`ns1.get.art` a `ns4.get.art`): ahí se crea ese TXT tal cual.
  4. Volver a GitHub y pulsar **Verify**. Si falla, se espera a que el DNS propague y se repite.
